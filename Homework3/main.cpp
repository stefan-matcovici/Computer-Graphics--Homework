...
class CFigura1 {
public:
	void figura1(double lungime, int nivel, CPunct &p, int d) {
		CVector v(1.0, 0.0);
		if (nivel == 0)
		{
			double lungimeJumatateDiagonala = lungime * sqrt(2) / 6.0;
			double lungimeLatura = lungime / 3.0;

			v.rotatie(135);
			CPunct start = v.getDest(p, lungimeJumatateDiagonala);
			v.rotatie(-135);
		...
		{
			double d1 = lungime * sqrt(2) / 3.0;
			double d2 = lungime / 3.0;
			...
			figura1(lungime, 0, p, d);

			for (int i = 0; i < 8; i++) {
				punctUrmator = v.getDest(p, i % 2 ? d1 : d2);
				figura1(lungimeLatura, nivel - 1, punctUrmator, d);
...
void afisareText(char* c, const char * displayNivel, const char * displayName)
{
	sprintf(c, "%2d", nivel);
	glRasterPos2d(-0.98, -0.98);
	for (int i = 0; i < strlen(displayNivel); i++) {
		glutBitmapCharacter(GLUT_BITMAP_9_BY_15, displayNivel[i]);
	}
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, c[0]);
	glutBitmapCharacter(GLUT_BITMAP_9_BY_15, c[1]);
	...
	for (int i = 0; i < strlen(displayName); i++) {
		glutBitmapCharacter(GLUT_BITMAP_9_BY_15, displayName[i]);
	}
}

void Display5() {
	CFigura1 cfigura1;
	cfigura1.afisare(1, nivel);

	char c[3];
	const char* displayNivel = "Nivel=";
	const char* displayName = "figura 1";
	afisareText(c, displayNivel, displayName);
...
void Display6() {
	...
	char c[3];
	const char* displayNivel = "Nivel=";
	const char* displayName = "figura 2";
	afisareText(c, displayNivel, displayName);
...
void Display7() {
	CFigura3 cfigura3;
	cfigura3.afisare(0.8/ pow(2.0, (nivel-1)), nivel);

	char c[3];
	const char* displayNivel = "Nivel=";
	const char* displayName = "figura 3";
	afisareText(c, displayNivel, displayName);
