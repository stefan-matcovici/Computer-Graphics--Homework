...
public:
	...
	void display() {
		...
		double xmax = (double)width / (double)min;
		double ymax = (double)height / (double)min;


		for (double i = 0; i < LINES + 1; i++) {
			glBegin(GL_LINE_STRIP);
			glVertex2f(-1.0 / xmax, (2.0 / (double)LINES * i - 1) / ymax);
			glVertex2f(1.0 / xmax, (2.0 / (double)LINES * i - 1) / ymax);
			glEnd();
		}
		...
		for (double i = 0; i < COLUMNS + 1; i++) {
			glBegin(GL_LINE_STRIP);
			glVertex2f((2.0 / (double)COLUMNS * i - 1) / xmax, -1.0 / ymax);
			glVertex2f((2.0 / (double)COLUMNS * i - 1) / xmax, 1.0 / ymax);
			glEnd();
		}
	}
	...
	void deseneazaLinie(int x0, int y0, int xn, int yn) {
		...
		std::vector<Punct> list = deseneazaLinieAlgoritm(x0, y0, xn, yn);
	...
	std::vector<Punct> deseneazaLinieAlgoritm(int x0, int y0, int xn, int yn) {
		if (abs(yn - y0) < abs(xn - x0)) {
			if (x0 > xn)
				return deseneazaLinieX(xn, yn, x0, y0);
			else
				return deseneazaLinieX(x0, y0, xn, yn);
		}
		else
		{
			if (y0 > yn)
				return deseneazaLinieY(xn, yn, x0, y0);
			else
				return deseneazaLinieY(x0, y0, xn, yn);
		}
				
		return deseneazaLinieX(yn, y0, xn, x0);
	}

	std::vector<Punct> deseneazaLinieX(int x0, int y0, int xn, int yn)
	{
	...
	std::vector<Punct> deseneazaLinieY(int x0, int y0, int xn, int yn)
	{
		std::vector<Punct> puncte;
...
void Display(void) {
	...
	grilaCarteziana.deseneazaLinie(0, 0, 6, 2);

	grilaCarteziana.deseneazaLinie(5, 2, 6, 8);
